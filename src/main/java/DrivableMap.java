...
class DrivableMap {
     ...
     */
    boolean addDrivable(String id, Drivable object) {
        if (this.drivable_map.containsKey(id)) return false;
    ...
    List<Tradable> getTradable() {
        List<Tradable> lstTradable = new ArrayList<>();

        for (Drivable drivable : this.drivable_map.values())
            if (drivable instanceof Tradable)
                lstTradable.add((Tradable) drivable);
